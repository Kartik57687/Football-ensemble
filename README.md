...
`python run_pipeline.py` at seed 42, taken from
[`outputs/metrics/test_metrics.json`](outputs/metrics/test_metrics.json) —
the most recent verified run:

| Model | Accuracy (this repo) | Macro F1 (this repo) | Weighted F1 (this repo) | Accuracy (paper) |
|---|---|---|---|---|
| XGBoost | 89.97% | 88.72% | 89.90% | 84.77% |
| BiLSTM | 88.31% | 86.69% | 88.19% | 88.16% |
| 1D-CNN | 37.17% | 36.79% | 38.04% | 46.68% |
| **Ensemble (LR)** | **89.60%** | **88.38%** | **89.60%** | **88.78%** |

The headline ensemble number reproduces within ~0.8 points, and nine of the
...
## Research Gaps & Current Extensions

This reproduction is being extended under the supervision of **Dr. Jitendra
Tembhurne** at IIIT Nagpur. The work below goes beyond reproducing the paper —
it probes weaknesses in the published method itself. Full detail, including the
diagnostic plots, is in [`docs/research_gaps.md`](docs/research_gaps.md).

| Direction | Status |
|---|---|
| CNN overfitting diagnosis | ✅ Completed (diagnostic run analysed) |
| Meta-learner leakage check (OOF vs. in-sample) | ✅ Completed (inconclusive result) |
| CNN architecture/regularisation fix | 🔄 In progress |
| Systematic hyperparameter search | 🔬 Planned |
| Chronological (walk-forward) validation | 🔬 Planned |
| Explainability (SHAP) | 🔬 Planned |
| LLM-based natural-language reasoning over predictions | 🔬 Planned |
| Dynamic in-play (live) prediction | 🔬 Planned |

### 1. The CNN overfits — confirmed with training curves

The 37.17% CNN accuracy above is not just a weak result, it has a diagnosable
cause. Training and validation loss/accuracy were tracked over 30 epochs on a
held-out validation split:

- **Loss:** training loss falls steadily from ≈1.10 to ≈0.69, while validation
  loss falls only briefly, bottoms out around epoch 10–15, then climbs back to
  ≈1.26 by epoch 30.
- **Accuracy:** training accuracy climbs from ≈37% to 69.4% by epoch 30.
  Validation accuracy peaks at ≈44.5% around epoch 15, then *declines* to
  ≈38.3% by epoch 30.

Rising validation loss alongside falling validation accuracy, while both
training curves keep improving, is the textbook signature of overfitting — the
CNN is increasingly memorising the training set rather than generalising.
Candidate fixes, not yet applied to the production model:

- **Early stopping** around epoch 15, where validation accuracy peaked.
- **Stronger dropout**, to reduce reliance on individual filters/neurons.

Feature ordering, input representation, and architecture depth remain open
questions (see `unspecified_details.md` #4 and #9) and have not been ruled out
as contributing causes.

### 2. Meta-learner training leakage — investigated, not confirmed

Algorithm 1, as published, trains the meta-learner on **in-sample** base-model
predictions (see `unspecified_details.md` #5) — the base models have already
seen the samples they're predicting on when generating meta-features. To check
whether this materially inflates the ensemble's reported accuracy, an
**out-of-fold (OOF)** variant was run: base learners generate predictions only
for samples they did *not* train on, and the meta-learner is fit on those
instead.

| Approach | Test accuracy |
|---|---|
| In-sample (current default) | 89.60% |
| Out-of-fold | 90.47% |
| Difference | +0.87 pts |

The OOF variant scored slightly *higher*, not lower — the opposite of what
naive leakage inflation would predict. **A single experiment with a 0.87-point
gap is not strong evidence of material leakage either way.** OOF training is
still the methodologically safer default for any future extension, and it is
already wired up as `config.USE_OUT_OF_FOLD_META_FEATURES = True` (see
Configuration below) — but this repository does not claim the leakage question
is settled.

### 3. No systematic hyperparameter optimisation

`src/config.py` uses one fixed set of hyperparameters per model, taken directly
from the paper's stated values. There is no evidence these are optimal for
*this* dataset:

- **XGBoost:** `learning_rate`, `n_estimators`, `max_depth`, `subsample`,
  `colsample_bytree`, `min_child_weight`, `gamma`
- **BiLSTM:** `learning_rate`, `units`, number of layers, `dropout`,
  `batch_size`, `epochs`
- **CNN:** `learning_rate`, `filters`, `kernel_size`, number of layers,
  `dropout`, `batch_size`, `epochs`

Planned direction: an [Optuna](https://optuna.org/) search over each model's
space, using validation performance as the objective, rather than manual tuning.

### 4. No chronological validation

The current split is an 80:20 **stratified random** split (`unspecified_details.md`
#3), which can place 2019–2020 matches in training while 2014–2015 matches land
in the test set. That does not match how the model would actually be used —
predicting a *future* match using only *past* information.

Planned direction: **walk-forward (rolling-origin) validation** — e.g. train on
2014–2017, test on 2018; train on 2014–2018, test on 2019; and so on — to get a
more realistic estimate of generalisation to genuinely future matches, and to
check whether performance is stable across seasons rather than an artifact of
the random split.

### 5. No explainability layer

Nothing in this repository currently explains *why* a prediction was made.
Planned direction: integrate **SHAP** for per-prediction feature attribution
(which features pushed the prediction toward Win/Draw/Loss, and by how much),
as a foundation for the LLM-reasoning direction below.

### 6. LLM-based reasoning over predictions (early-stage direction)

Long-term goal: convert model output + SHAP attributions into a human-readable
explanation, e.g.

```
Model prediction: Home Team Win
      → SHAP feature attribution (which features, how much)
      → LLM
      → Plain-language explanation
```

**Nothing here is implemented yet.** This is listed as a direction under
active discussion with Dr. Tembhurne, not a shipped feature — do not read the
diagram above as describing existing code.

### 7. No dynamic / in-play prediction

The model produces a single **pre-match** probability and does not update as a
match unfolds. A possible extension is a live model that ingests in-match
events (goals, cards, substitutions, shots, possession swings) as a time
sequence — a natural fit for a temporal model like the BiLSTM — and revises the
Win/Draw/Loss probabilities as the match progresses, rather than only at kickoff.
